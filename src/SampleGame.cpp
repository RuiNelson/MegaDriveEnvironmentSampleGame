...
void SampleGame::onHSync(int scanline) {
    // Each horizontal line gets its own Plane B offset. Advancing the phase
    // moves the shape through the background instead of shaking Plane A as a
    // whole; the HUD, banner, gameplay text and sprites therefore stay fixed.
    const auto step = static_cast<unsigned>((scanline + backgroundWavePhase_) & 0x3F);
    int offset;
    if (step < 16u) {
    ...
    vdp::writeHorizontalScrollLine(memory_, scanline, 0, static_cast<std::uint16_t>(offset));
}
