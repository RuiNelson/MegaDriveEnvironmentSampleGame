...
The real Mega Drive build additionally requires:

- [vasm](http://sun.hasenbraten.de/vasm), built as `vasmm68k_std`;
- `m68k-elf-g++`, `m68k-elf-ld`, and `m68k-elf-objcopy`.

On macOS, install the GNU cross tools with:

```bash
brew install m68k-elf-binutils m68k-elf-gcc
```

Download the official vasm source release, then build and install its M68k
standard-syntax executable:
